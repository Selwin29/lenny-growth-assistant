...
Wired to the application's own Settings/Base so migrations always run
against the same database and are aware of every model — no separate
...
import app.models  # noqa: F401  (imports User, ChatSession, Message, Artifact)

# Alembic Config object, providing access to values in alembic.ini.
config = context.config

# Override the sqlalchemy.url from alembic.ini with the app's own
# DATABASE_URL so there's a single source of truth for DB connection info.
